...
const getHabitsFromPage = async (notion, page) => {
  ...
  const properties = habitKeys.map((key) => {
...
const mergeDailyNote = async (directory, properties) => {
  ...
  const existHabits = ast.children.find((item) => item.type === 'heading' && item.children[0]?.value === 'Habits');
  ...
  const existMemoHeading = ast.children.find((item) => item.type === 'heading' && item.children[0]?.value === 'Memo');

  const insertBeforeMemo = (acc, item, i, array) => {
    if (array[i+1]?.type === 'heading' && array[i+1].children[0]?.value === 'Memo') {
  ...
  const children = existMemoHeading ? ast.children.reduce(insertBeforeMemo, []) : [...ast.children, habitsHeadingAst ,habitsAst];
  ...
  fs.writeFileSync(obsidianDailyNoteFilename, replacer(toMarkdown(afterAst, options)));
}
